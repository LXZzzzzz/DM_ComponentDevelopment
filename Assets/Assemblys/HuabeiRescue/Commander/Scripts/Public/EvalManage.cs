...
namespace ReportGenerate
{
    public class EvalManage
    {
        ...
        public ResultFireWaterData EvalWaterCompute(ResultFireWaterOutData outdata, ResultFireWaterSystemData sysData)
        {
            ...
            foreach (KeyValuePair<HeliData, List<HeliSortieData>> item in outdata.HeliSortieWaterDataList)
            {
                for (int i = 0; i < item.Value.Count; i++)
                {
                    ...
                    item.Key.单机投水总重量 += MaxWaterWeight;
        ...
        public ResultMaterialPersonData EvalMaterialCompute(ResultMaterialPersonOutData outData, ResultRescueSystemData sysData)
        {
            ...
            {
                double valMaterialNeed = item.MaterialWeight * sysData.人均救援物资需求;
                if (item.MaterialWeight >= valMaterialNeed)
                {
                    item.MaterialDegree = 1;
                }
                else
                {
                    item.MaterialDegree = item.MaterialWeight / valMaterialNeed;
                }

                data.任务结束时各安置点数据.Add(item);


                valAwZong += item.MaterialDegree * valMaterialNeed;
            }
            ...
            foreach (KeyValuePair<HeliData, List<HeliSortieData>> item in outData.HeliSortieMaterialPersonDataList)
            {
                for (int i = 0; i < item.Value.Count; i++)
                {
                    double zPtime = item.Value[i].ReturnTime - item.Value[i].TakeOffTime - (item.Value[i].MaterialPointTime - item.Value[i].TakeOffTime);
                    ...
                    item.Key.单机任务成本 += (item.Key.Price * 0.00029453 + item.Key.Consumption * 0.00087035 + 0.965) * (item.Value[i].ReturnTime - item.Value[i].TakeOffTime);
            ...
            data.协同指挥效能 = 0.6 * data.人员转运总体任务效率 + 0.4 * data.物资投放总体任务效率;

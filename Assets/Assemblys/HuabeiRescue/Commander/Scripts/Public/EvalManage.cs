...
namespace ReportGenerate
{
    public class EvalManage
    {
        ...
        public ResultFireWaterData EvalWaterCompute(ResultFireWaterOutData outdata, ResultFireWaterSystemData sysData)
        {
            ...
            {
                double valWaterNeed = (item.burnedArea - (item.initBurnedArea - item.initBurnArea)) * sysData.单位燃烧面积投水需求;
            ...
            foreach (KeyValuePair<HeliData, List<HeliSortieData>> item in outdata.HeliSortieWaterDataList)
            {
                for (int i = 0; i < item.Value.Count; i++)
                {
                    ...
                    item.Key.单机任务成本 += (item.Key.Price * 0.00029453 + item.Key.Consumption * 0.00087035 + 0.965) * (item.Value[i].EndMissonTime - item.Value[i].TakeOffTime);
                }
            ...
            data.成本基数 = (sysData.最低直升机单价 * 0.00029453 + sysData.最低小时燃油消耗率 * 0.00087035 + 0.965) * ((outdata.取水点到投水点的最短路径 * 2) / sysData.最大巡航速度 + sysData.单次取水和投水时间) * (ZongWaterWeight / sysData.吊桶单次最大装载量);
        ...
        public ResultMaterialPersonData EvalMaterialCompute(ResultMaterialPersonOutData outData, ResultRescueSystemData sysData)
        {
            ...
            foreach (KeyValuePair<HeliData, List<HeliSortieData>> item in outData.HeliSortieMaterialPersonDataList)
            {
                for (int i = 0; i < item.Value.Count; i++)
                {
                    ...
                    double MaxPersonCount = (zPtime / (outData.救援点到安置点的最短路径 / sysData.最大巡航速度 + sysData.单次人员吊救时间)) * sysData.直升机单次最大运载人数;
                    item.Key.单机人员转运效率.Add(item.Value[i].PersonCount / MaxPersonCount);
                    double MaxMaterialWeight = (zMtime / (outData.物资装载起降点到安置点的最短路径 / sysData.最大巡航速度 + sysData.单次物资投放时间)) * sysData.人均救援物资需求;
            ...
            data.人员转运最短时间 = (outData.救援点到安置点的最短路径 / sysData.最大巡航速度 + sysData.单次人员吊救时间) * sysData.受灾需转运总人数 / sysData.直升机单次最大运载人数;
            data.物资投放最短时间 = (outData.物资装载起降点到安置点的最短路径 / sysData.最大巡航速度 + sysData.单次物资投放时间) * data.任务结束时对应的物资投放总需求 / sysData.人均救援物资需求;
            ...
            data.人员转运任务成本基数 = (sysData.最低直升机单价 * 0.00029453 + sysData.最低小时燃油消耗率 * 0.00087035 + 0.965) * (outData.救援点到安置点的最短路径 / sysData.最大巡航速度 + sysData.单次人员吊救时间) * (outData.任务结束时转运总人数 / sysData.直升机单次最大运载人数);

            data.物资投放任务运成本基数 = (sysData.最低直升机单价 * 0.00029453 + sysData.最低小时燃油消耗率 * 0.00087035 + 0.965) * (outData.物资装载起降点到安置点的最短路径 / sysData.最大巡航速度 + sysData.单次物资投放时间) * (ZongMaterialWeight / sysData.直升机单次最大运载物资重量);

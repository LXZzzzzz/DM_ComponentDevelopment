...
namespace ReportGenerate
{
    public class PDFReport
    {
            ...
            Dictionary<int, List<string>> tqData)
        {
            ...
            commander1.AddCell(MyCell(scores == null ? "--" : $"{scores.qrzqxx}分", 1, 2));
            commander1.AddCell(MyCell(scores == null ? "--" : $"{scores.qrzqxx_zg}分", 1, 2));
            //值
            ...
            commander1.AddCell(MyCell($"确认出动装备信息", 2, personAss.yjzhy.cdjyll.Count + 1));
            commander1.AddCell(MyCell("机型", 2, 1));
            commander1.AddCell(MyCell("编号", 2, 1));
            commander1.AddCell(MyCell("数量", 2, 1));
            commander1.AddCell(MyCell("", 2, 1));
            commander1.AddCell(MyCell(scores == null ? "--" : $"{scores.cdzbxxqr}分", 1, personAss.yjzhy.cdjyll.Count + 1));
            commander1.AddCell(MyCell(scores == null ? "--" : $"{scores.cdzbxxqr_zg}分", 1, personAss.yjzhy.cdjyll.Count + 1));
            //值
            ...
            commander1.AddCell(MyCell($"确认出动人员信息", 2, personAss.yjzhy.cdjyll.Count + 1));
            commander1.AddCell(MyCell("机型编号", 2, 1));
            commander1.AddCell(MyCell("机组", 2, 1));
            commander1.AddCell(MyCell("保障组", 2, 1));
            commander1.AddCell(MyCell("", 2, 1));
            commander1.AddCell(MyCell(scores == null ? "--" : $"{scores.cdryxxqr}分", 1, personAss.yjzhy.cdjyll.Count + 1));
            commander1.AddCell(MyCell(scores == null ? "--" : $"{scores.cdryxxqr_zg}分", 1, personAss.yjzhy.cdjyll.Count + 1));
            //值
            for (int i = 0; i < personAss.yjzhy.cdjyll.Count; i++)
            {
                int index = i;
            ...
            commander1.AddCell(MyCell(scores == null ? "--" : $"{scores.xdrw}分", 1, 2));
            commander1.AddCell(MyCell(scores == null ? "--" : $"{scores.xdrw_zg}分", 1, 2));
            commander1.AddCell(MyCell(personAss.yjzhy.rwjl, 8, 2));
            ...
            PdfPTable commander2 = new PdfPTable(10);
            commander2.AddCell(MyCell("评分规则：每项训练点满分10分，未完成：0分，较差：1~3分，一般：4~6分，较好：7~9分", 10, 1));
            commander2.AddCell(MyCell1("评分项", 2, 1));
            commander2.AddCell(MyCell1("评分内容细节", 6, 1));
            commander2.AddCell(MyCell1("分值", 1, 1));
            commander2.AddCell(MyCell1("主观打分", 1, 1));
            //表头结束
            commander2.AddCell(MyCell($"领受任务", 2, 1));
            ...
            commander2.AddCell(MyCell($"确认机长的特情处置报告", 2, 2));
            commander2.AddCell(MyCell("报告次数", 3, 1));
            commander2.AddCell(MyCell($"{(tqData.ContainsKey(2) ? tqData[2].Count : 0).ToString()}", 3, 1));
            commander2.AddCell(MyCell(scores == null ? "--" : $"{scores.qrtqczbg}分", 1, 2));
            commander2.AddCell(MyCell(scores == null ? "--" : $"{scores.qrtqczbg_zg}分", 1, 2));
            ...
            for (int i = 0; i < personAss.sjzhy.Count; i++)
            {
                ...
                else paLevel3i = new Paragraph("  (" + (index + 1) + ") " + personAss.sjzhy[i].jzname + "   得分：" + jzScore.jzZhyTotalScore.ToString("F2"), fontSub);
                ...
                commander3.AddCell(MyCell1("评分内容细节", 6, 1));
                commander3.AddCell(MyCell1("分值", 1, 1));
                ...
                commander3.AddCell(MyCell("特情次数", 3, 1));
                commander3.AddCell(MyCell($"{(tqData.ContainsKey(1) ? tqData[1].Count : 0).ToString()}", 3, 1));
                commander3.AddCell(MyCell(jzScore == null ? "--" : $"{jzScore.xxczhybg}分", 1, 2));
            ...
            tableResult.AddCell(MyCell($"任务效能总分 {WaterEval.ToString("0.00")}", 4, 1));
            tableResult.AddCell(MyCell("灭火任务完成度", 2, 1));
            tableResult.AddCell(MyCell(WaterMissonDegree.ToString("0.00") + " %", 2, 1));
            tableResult.AddCell(MyCell("灭火任务总时间效率", 2, 1));
            tableResult.AddCell(MyCell(resultData.灭火任务总时间效率.ToString("0.00"), 2, 1));
            ...
            for (int i = 0; i < personAss.sjzhy.Count; i++)
            {
                ...
                tableResult.AddCell(MyCell((zongWcd / rws.Count * 100).ToString("0.00") + "%", 2, 1));
            }
            ...
            PersonAssessment_ResultFireWater personAss, ScoreStatistics scores, Dictionary<int, List<string>> tqData)
        {
            ...
            commander1.AddCell(MyCell("灾害类型", 2, 1));
            commander1.AddCell(MyCell("灾情规模", 2, 1));
            commander1.AddCell(MyCell("物资投放重量(千克)", 2, 1));
            commander1.AddCell(MyCell("待转运人数", 2, 1));
            commander1.AddCell(MyCell(scores == null ? "--" : $"{scores.qrzqxx}分", 1, 2));
            commander1.AddCell(MyCell(scores == null ? "--" : $"{scores.qrzqxx_zg}分", 1, 2));
            //值
            commander1.AddCell(MyCell(personAss.yjzhy.zhlx, 2, 1));
            commander1.AddCell(MyCell(personAss.yjzhy.zqgm, 2, 1));
            commander1.AddCell(MyCell(personAss.yjzhy.wztfzl.ToString(), 2, 1));
            commander1.AddCell(MyCell(personAss.yjzhy.dzyrs.ToString(), 2, 1));
            ...
            PdfPTable tableResult = new PdfPTable(4);
            tableResult.AddCell(MyCell($"任务效能总分 {RescueEval.ToString("0.00")}", 4, 1));
            tableResult.AddCell(MyCell($"人员转运总体任务效率    {PersonZongEfficiency.ToString("0.00")}", 2, 1));
            tableResult.AddCell(MyCell($"物资投放总体任务效率    {MaterialZongEfficiency.ToString("0.00")}", 2, 1));
            tableResult.AddCell(MyCell($"人员转运总体任务效率", 4, 1));
            tableResult.AddCell(MyCell("人员转运任务完成度", 2, 1));
            tableResult.AddCell(MyCell(PersonDegree.ToString("0.00") + " %", 2, 1));
            tableResult.AddCell(MyCell("人员转运任务时间效率", 2, 1));
            tableResult.AddCell(MyCell(PersonTimeEfficiency.ToString("0.00"), 2, 1));
            tableResult.AddCell(MyCell("物资投放总体任务效率", 4, 1));
            tableResult.AddCell(MyCell("物资投放任务完成度", 2, 1));
            tableResult.AddCell(MyCell(MaterialDegree.ToString("0.00") + " %", 2, 1));
            tableResult.AddCell(MyCell("物资投放任务时间效率", 2, 1));
            tableResult.AddCell(MyCell(MaterialTimeEfficiency.ToString("0.00"), 2, 1));
            tableResult.AddCell(MyCell("机组任务完成度", 4, 1));
            for (int i = 0; i < personAss.sjzhy.Count; i++)
            {
                ...
                tableResult.AddCell(MyCell((zongWcd2 / rws2.Count * 100).ToString("0.00") + "%", 2, 1));
            }

...
Shader "Custom/ForceShield_1"
{
	Properties
	{
		[HideInInspector] _AlphaCutoff("Alpha Cutoff ", Range(0, 1)) = 0.5
		[ASEBegin]_IntersectIntensity("Intersect Intensity", Range( 0 , 6)) = 1
		_Opacity("Opacity", Float) = 1
		[HDR]_Color("Color", Color) = (0,0.5279799,1,0)
		_Bias("Bias", Float) = 0
	...
	SubShader
	{
		...
		Pass
		{
					...
					)
			{
				...
				float fresnelNdotV29 = dot( ase_worldNormal, V );
				float fresnelNode29 = ( _Bias + _Scale * pow( 1.0 - fresnelNdotV29, _Power ) );
				float4 screenPos = packedInput.ase_texcoord2;
				float4 ase_screenPosNorm = screenPos / screenPos.w;
				ase_screenPosNorm.z = ( UNITY_NEAR_CLIP_VALUE >= 0 ) ? ase_screenPosNorm.z : ase_screenPosNorm.z * 0.5 + 0.5;
				float screenDepth3 = LinearEyeDepth(SampleCameraDepth( ase_screenPosNorm.xy ),_ZBufferParams);
				float distanceDepth3 = abs( ( screenDepth3 - LinearEyeDepth( ase_screenPosNorm.z,_ZBufferParams ) ) / ( _IntersectIntensity ) );
				float clampResult17 = clamp( ( 1.0 - distanceDepth3 ) , 0.0 , 1.0 );
				float temp_output_31_0 = ( fresnelNode29 + clampResult17 );
				float4 temp_output_30_0 = ( _Color * temp_output_31_0 );
				
				float clampResult34 = clamp( temp_output_31_0 , 0.0 , 1.0 );
				
				surfaceDescription.Color = temp_output_30_0.rgb;
				surfaceDescription.Emission = temp_output_30_0.rgb;
				surfaceDescription.Alpha = ( ( _Color.a * _Opacity ) * clampResult34 );
        ...
        Pass
		{
					...
					)
			{
				...
				float3 ase_worldViewDir = ( _WorldSpaceCameraPos.xyz - ase_worldPos );
				ase_worldViewDir = normalize(ase_worldViewDir);
				float fresnelNdotV29 = dot( packedInput.normalWS, ase_worldViewDir );
				float fresnelNode29 = ( _Bias + _Scale * pow( 1.0 - fresnelNdotV29, _Power ) );
...
ASEEND*/
//CHKSM=92D20DEBB5C804C0A34E3F0F01895B0DBD0E7884